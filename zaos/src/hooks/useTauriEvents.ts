    ...
    const setupListeners = async () => {
      ...
      const workflowChangeListener = await listen(
        ...
        (event) => {
          ...
          if (payload.phase) workflowStore.setPhase(payload.phase);
          if (payload.epic) workflowStore.setEpic(payload.epic);
          if (payload.task) workflowStore.setTask(payload.task);
          if (payload.mode) workflowStore.setMode(payload.mode);
